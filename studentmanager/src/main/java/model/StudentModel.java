...
public class StudentModel {
	...
	public int insert(int id,String name,String sex,int age,String grade,float score,String fileName){
		int a=0;
		try {
			Connection conn=s.getConnection();
	...
	public int update2(int id, String name, String sex, int age, String grade, float score, String fileName) {
		int a=0;
		try {
			Connection conn=s.getConnection();
			String sql="update student set name=?,sex=?,age=?,grade=?,score=?,fileName=? where id=?";
			ps=conn.prepareStatement(sql);
			ps.setInt(7, id);
			ps.setString(1, name);
			ps.setString(2, sex);
